...
/** @type {(remote: unknown) => import("./types.d.ts").Remote} */
const sanitizeRemote = (remote) => {
  if (typeof remote !== "object" || remote === null)
    throw new Error("Invalid frontmatter: remote is not an object", {
      cause: { remote },
    });
  if (!("name" in remote) || typeof remote.name !== "string")
    throw new Error("Invalid frontmatter: remote.name is not a string", {
      cause: { remote },
    });
  if (!("link" in remote) || typeof remote.link !== "string")
    throw new Error("Invalid frontmatter: remote.link is not a string", {
      cause: { remote },
    });
  const { name, link } = remote;
  return { name, link };
};
...
const sanitizeEventFrontmatter = (data) => {
  ...
  const remote = "remote" in data ? sanitizeRemote(data.remote) : null;
  return { uid, layout, link, title, short_title, time, venue, remote };
};
